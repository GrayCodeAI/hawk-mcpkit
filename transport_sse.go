...
// header are rejected. The two modes are mutually exclusive.
//
...
func (s *Server) ServeSSE(addr string) error {
	...
	// If HTTP token auth is configured, wrap the SSE server (which
	// implements http.Handler) with the token-checking handler and serve
	// via a custom http.Server. Otherwise, let the SSE server manage its
	// own listener.
	if s.httpToken != "" {
		srv := &http.Server{
			Addr:    addr,
			Handler: httpTokenHandler(s.httpToken, sseServer),
		}
...
// object exists), so the caller owns the lifecycle.
...
func (s *Server) buildSSEServer(addr string) (*mcpserver.SSEServer, error) {
	...
	return mcpserver.NewSSEServer(s.mcp, opts...), nil
}
